...
import { applyRule, estimateWasteSaved, estimateCO2Saved } from '@/lib/rules'
import type { ItemType, Condition } from '@/types'

// Seed dummy historical data for demonstration
export async function POST(_request: NextRequest): Promise<NextResponse> {
  ...
  const records = dummyData.map((d) => {
    ...
    return {
      ...
      action,
      recommendation,
      estimatedWaste: estimateWasteSaved(d.itemType),
      estimatedCO2: estimateCO2Saved(d.itemType),
    }
  })
